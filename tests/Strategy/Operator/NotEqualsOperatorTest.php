...
final class NotEqualsOperatorTest extends TestCase
{
    ...
    public function it_returns_correct_operator_type(): void
    {
        ...
        // Assert
        self::assertSame(AttributeOperator::NOT_EQUALS, $result);
    }

    #[Test]
    #[DataProvider('provideNonEqualValues')]
    public function it_returns_true_for_non_equal_values(mixed $actual, mixed $expected): void
    {
        // Arrange
        $operator = new NotEqualsOperator();
    ...
    public static function provideNonEqualValues(): iterable
    {
        ...
        yield 'different arrays' => [['a'], ['b']];
    }

    #[Test]
    #[DataProvider('provideEqualValues')]
    public function it_returns_false_for_equal_values(mixed $actual, mixed $expected): void
    {
        // Arrange
        $operator = new NotEqualsOperator();
    ...
    public function it_uses_strict_comparison(): void
    {
        ...
        self::assertTrue($operator->evaluate(1, '1'));
        self::assertTrue($operator->evaluate(1, true));
        self::assertTrue($operator->evaluate(0, false));
        self::assertTrue($operator->evaluate(0, ''));
        self::assertTrue($operator->evaluate(null, false));
    }
    ...
    public function it_handles_unicode_strings(): void
    {
        ...
        // Act & Assert
        self::assertFalse($operator->evaluate('Привет', 'Привет'));
        self::assertTrue($operator->evaluate('Привет', 'привет'));
        self::assertFalse($operator->evaluate('日本語', '日本語'));
        self::assertTrue($operator->evaluate('日本語', '中文'));
    }
    ...
    public function it_handles_empty_values(): void
    {
        ...
        self::assertFalse($operator->evaluate('', ''));
        self::assertTrue($operator->evaluate('', ' '));
        self::assertTrue($operator->evaluate(null, ''));
        self::assertFalse($operator->evaluate(null, null));
    }
}
